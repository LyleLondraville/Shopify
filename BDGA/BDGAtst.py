...
def parse(dict, site):
	...
	tempdata = 'Brand : %s \n\nTitle : %s \n\nCategory : %s \n\nPrice : %s \n\nURL : %s \n\nImage : %s \n\nDescription : %s \n\n---------------Product Variations--------------\n\n' % (vendor, title, category, price, url, image, desk)
	...
	while len(varIndex) > 0 :
		...
		for var in varIndex:
			...
			if var not in whileVarIndex:
								...
								try :
									...
									if whileATC.status_code == 200 :
										...
										if JSONdata['product_id'] == PID :
											...
											currentVarList.update({str(varIndex[whileIndexCont]):JSONdata['variant_title']})
									...
									print 'Error excpt1 sleeping for 200 seconds'
									time.sleep(200)
							else :
								index404 += 450
				...
				print 'Error excpt3 sleeping for 200 seconds'
				time.sleep(200)
...
scrape('https://shop.bdgastore.com', 26657146120, 26657175304, '1', 'Boudega')